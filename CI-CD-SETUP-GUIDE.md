...
### 问题 1：流水线失败，提示 "npm ci 失败"

**原因**：常见原因是 `package-lock.json` 未同步、Node 版本不一致，或缓存污染。

**解决方法**：

在 `.gitee/workflows/ci.yml` 中已经切换为基于 `npm` 的安装方式：
...
1. `package-lock.json` 是否已提交且与 `package.json` 一致
2. 流水线 Node 版本是否与项目要求一致
3. 是否存在历史缓存导致依赖解析异常
...
CI 配置中已经改为 npm 安装模式，建议结合 npm 缓存目录提升依赖安装速度。
