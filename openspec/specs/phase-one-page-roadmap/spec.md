...
### Requirement: The phase-one roadmap SHALL formalize IOT as the next telemetry-support batch after EAM

The phase-one roadmap SHALL explicitly transition from the approved first-batch `EAM` pages into a first-batch `IOT` telemetry-support baseline before opening broader `BI` or governance-oriented pages.

#### Scenario: IOT follows EAM in the approved roadmap

- **WHEN** the team finishes the current approved `EAM` batch and decides the next module batch
- **THEN** `IOT` MUST be the next recommended module batch
- **AND** that batch MUST begin from `设备连接管理`, `点位配置`, `实时监控`, and `报警记录`

#### Scenario: IOT batch starts from approved equipment-operation baselines

- **WHEN** the roadmap reviews whether `IOT` pages are ready to begin
- **THEN** the review MUST first confirm that upstream equipment-operation pages that intersect with telemetry support are already stabilized where needed
- **AND** `IOT` pages MUST inherit those approved cross-module baselines rather than redefining equipment asset, production execution, warehouse, or shared master-data truth

### Requirement: The phase-one roadmap SHALL keep automation and analysis pages out of the first IOT batch

The phase-one roadmap SHALL separate the first telemetry-support batch from later production automation, advanced protocol integration, and analysis pages in `IOT`.

#### Scenario: First-batch IOT pages stay focused on telemetry support

- **WHEN** the team reviews the first `IOT` static-page batch for scope control
- **THEN** the batch MUST remain limited to `设备连接管理`, `点位配置`, `实时监控`, and `报警记录`
- **AND** pages such as `自动报工`, protocol-driver integration, and telemetry analysis dashboards MUST wait for later expansion batches

### Requirement: The phase-one roadmap SHALL formalize BI as the next analytics batch after IOT

The phase-one roadmap SHALL explicitly transition from the approved first-batch `IOT` pages into a first-batch `BI` analytics baseline before opening system-governance pages.

#### Scenario: BI follows IOT in the approved roadmap

- **WHEN** the team finishes the current approved `IOT` batch and decides the next module batch
- **THEN** `BI` MUST be the next recommended module batch
- **AND** that batch MUST begin from `生产分析看板`, `质量分析看板`, `库存分析看板`, and `经营分析看板`

#### Scenario: BI batch starts from approved source-object baselines

- **WHEN** the roadmap reviews whether `BI` pages are ready to begin
- **THEN** the review MUST first confirm that upstream production, quality, inventory, business, equipment, and telemetry pages that feed analysis are already stabilized where needed
- **AND** `BI` pages MUST inherit those approved cross-module baselines rather than redefining execution, quality, inventory, order, finance, equipment, or telemetry truth

### Requirement: The phase-one roadmap SHALL keep topic analytics out of the first BI batch

The phase-one roadmap SHALL separate the first analytics batch from later topic analytics and generic report management pages in `BI`.

#### Scenario: First-batch BI pages stay focused on integrated business-chain analytics

- **WHEN** the team reviews the first `BI` static-page batch for scope control
- **THEN** the batch MUST remain limited to `生产分析看板`, `质量分析看板`, `库存分析看板`, and `经营分析看板`
- **AND** pages such as `能耗`, `安环`, `人资`, and generic `经营报表` MUST wait for later expansion batches
