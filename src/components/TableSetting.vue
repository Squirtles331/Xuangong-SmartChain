  ...
  <div class="table-setting" :class="{ 'table-setting--fullscreen': isFullscreen }">
    <div class="table-setting__toolbar">
      ...
      <el-space wrap :size="8">
        ...
        <el-popover v-if="showColumnSetting" placement="bottom-end" :width="180" trigger="click" transition="el-zoom-in-top">
          <template #reference>
              ...
              <VueDraggable v-model="settingColumnList" :animation="150" handle=".table-setting__drag-handle">
                <div v-for="item in settingColumnList" :key="item.key" class="table-setting__draggable-item">
                  ...
                  <div class="table-setting__pins">
                    <span class="table-setting__pin-btn" :class="{ 'is-active': item.fixedLeft }" @click.stop="toggleFixedLeft(item.key)">
                      <el-icon :size="14">
                        <LocationFilled />
                      </el-icon>
                    </span>
                    <span
                      class="table-setting__pin-btn table-setting__pin-btn--right"
...
import { FullScreen, Grid, LocationFilled, Rank, RefreshRight, ScaleToOriginal, Setting, Switch } from '@element-plus/icons-vue'
...
function getColumnKey(column: TableColumnItem, index: number): string {
  if (column.prop != null && column.prop !== '') return String(column.prop)
  if (column.type) return `__type_${String(column.type)}_${index}__`
...
const initialSettingColumns = computed<TableSettingColumnItem[]>(() => {
  ...
  list.forEach((column, index) => {
    if (!isColumnInSettingList(column)) return
...
function isColumnStructureMatch(user: TableSettingColumnItem[], initial: TableSettingColumnItem[]): boolean {
  if (user.length === 0 || user.length !== initial.length) return false
...
watch(
  ...
  (next) => {
    ...
    if (!isColumnStructureMatch(settingColumnList.value, next)) settingColumnList.value = next.map((i) => ({ ...i }))
  },
  { immediate: true }
)
...
const settingColumns = computed<TableColumnItem[]>(() => {
  ...
  if (!settingColumnList.value.length) return prefix.length ? [...prefix] : []
