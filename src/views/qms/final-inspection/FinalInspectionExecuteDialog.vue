<template>
  <el-dialog v-model="visible" title="执行完工检验" width="760px" :lock-scroll="false">
    <el-descriptions :column="2" border>
      ...
      <el-descriptions-item label="完工单号">{{ task?.sourceCode }}</el-descriptions-item>
...
export interface FinalInspectionExecuteTask {
  code: string
  material: string
  sourceCode: string
  sourceName: string
...
export interface FinalInspectionExecuteItem {
  name: string
  standard: string
  value: string
}
...
defineProps<{
  task: FinalInspectionExecuteTask | null
}>()

const visible = defineModel<boolean>('visible', { required: true })
const items = defineModel<FinalInspectionExecuteItem[]>('items', { required: true })
...
function resolveItemTag(row: Record<string, any>) {
  const item = row as FinalInspectionExecuteItem
...
function isItemPass(row: FinalInspectionExecuteItem) {
