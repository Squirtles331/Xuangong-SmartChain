<template>
  ...
  >
    ...
    <template #dialog>
      <FinalInspectionFormDialog v-model:visible="dialogVisible" v-model:form="formModel" :mode="dialogMode" @submit="submitDialog" />
      <FinalInspectionExecuteDialog
...
import FinalInspectionExecuteDialog, { type FinalInspectionExecuteItem, type FinalInspectionExecuteTask } from './FinalInspectionExecuteDialog.vue'
import FinalInspectionFormDialog, { type FinalInspectionFormModel } from './FinalInspectionFormDialog.vue'

const pageTitle = '完工检验'
const pageCategory: InspectionCategory = 'final'
...
const searchColumns: FormColumnItem[] = [
  ...
  { type: 'input', label: '完工单号', field: 'sourceCode', props: { clearable: true } as any },
  {
    type: 'select-v2',
    label: '状态',
    field: 'status',
...
const formModel = ref<FinalInspectionFormModel>(createDefaultForm())
const executeVisible = ref(false)
const currentTask = ref<FinalInspectionExecuteTask | null>(null)
const executeItems = ref<FinalInspectionExecuteItem[]>([])
const executeResult = ref<Exclude<InspectionVerdict, ''>>('pass')

function createDefaultForm(): FinalInspectionFormModel {
  return {
    id: '',
    code: '',
    category: pageCategory,
