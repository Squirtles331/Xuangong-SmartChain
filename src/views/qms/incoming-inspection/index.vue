<template>
  ...
  >
    ...
    <template #dialog>
      <IncomingInspectionFormDialog v-model:visible="dialogVisible" v-model:form="formModel" :mode="dialogMode" @submit="submitDialog" />
      <IncomingInspectionExecuteDialog
...
import IncomingInspectionExecuteDialog, {
  type IncomingInspectionExecuteItem,
  type IncomingInspectionExecuteTask
} from './IncomingInspectionExecuteDialog.vue'
import IncomingInspectionFormDialog, { type IncomingInspectionFormModel } from './IncomingInspectionFormDialog.vue'

const pageTitle = '来料检验'
const pageCategory: InspectionCategory = 'incoming'
...
const searchColumns: FormColumnItem[] = [
  ...
  { type: 'input', label: '收货单号', field: 'sourceCode', props: { clearable: true } as any },
  {
    type: 'select-v2',
    label: '状态',
    field: 'status',
...
const formModel = ref<IncomingInspectionFormModel>(createDefaultForm())
const executeVisible = ref(false)
const currentTask = ref<IncomingInspectionExecuteTask | null>(null)
const executeItems = ref<IncomingInspectionExecuteItem[]>([])
const executeResult = ref<Exclude<InspectionVerdict, ''>>('pass')

function createDefaultForm(): IncomingInspectionFormModel {
  return {
    id: '',
    code: '',
    category: pageCategory,
