<template>
  <el-dialog v-model="visible" title="执行来料检验" width="760px" :lock-scroll="false">
    <el-descriptions :column="2" border>
      ...
      <el-descriptions-item label="收货单号">{{ task?.sourceCode }}</el-descriptions-item>
...
export interface IncomingInspectionExecuteTask {
  code: string
  material: string
  sourceCode: string
  sourceName: string
...
export interface IncomingInspectionExecuteItem {
  name: string
  standard: string
  value: string
}
...
defineProps<{
  task: IncomingInspectionExecuteTask | null
}>()

const visible = defineModel<boolean>('visible', { required: true })
const items = defineModel<IncomingInspectionExecuteItem[]>('items', { required: true })
...
function resolveItemTag(row: Record<string, any>) {
  const item = row as IncomingInspectionExecuteItem
...
function isItemPass(row: IncomingInspectionExecuteItem) {
