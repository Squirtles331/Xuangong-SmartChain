<template>
  ...
  >
    <template #channel="{ row }">
      <StatusTag :value="row.channel" :options="channelOptions" />
    </template>
...
import type { NotificationRule } from '@/api/system'
...
import { sysNotificationRuleRecords } from '../static-data'
import NotificationRuleFormDialog, { type NotificationRuleFormModel } from './NotificationRuleFormDialog.vue'

const channelOptions = [
  { label: '企业微信', value: 'wecom', type: 'success' as const },
  { label: '钉钉', value: 'dingtalk', type: 'primary' as const },
  { label: '站内消息', value: 'internal', type: 'warning' as const }
]
...
const searchColumns: FormColumnItem[] = [
  { type: 'input', label: '关键字', field: 'keyword', props: { placeholder: '业务类型 / Webhook 地址', clearable: true } as any },
...
const filteredRecords = computed(() =>
  sysNotificationRuleRecords.value.filter((item) => {
    const matchKeyword = !queryParams.keyword || item.bizType.includes(queryParams.keyword) || item.webhookUrl.includes(queryParams.keyword)
    const matchChannel = !queryParams.channel || item.channel === queryParams.channel
...
const { tableData, pagination, loading, search, refresh, onDelete } = useTable<NotificationRule>({
  ...
  deleteAPI: async (ids) => {
    sysNotificationRuleRecords.value = sysNotificationRuleRecords.value.filter((item) => !ids.includes(item.id))
  }
})
...
async function submitDialog() {
  ...
  if (dialogMode.value === 'add') {
    sysNotificationRuleRecords.value.unshift({
      id: `NOTICE-${String(sysNotificationRuleRecords.value.length + 1).padStart(3, '0')}`,
    ...
    sysNotificationRuleRecords.value = sysNotificationRuleRecords.value.map((item) =>
  ...
  await refresh()
  ElMessage.success(dialogMode.value === 'add' ? '已新增静态通知规则数据' : '已更新静态通知规则数据')
}

async function handleTest(row: NotificationRule) {
  ElMessage.success(`已发送静态测试消息：${row.bizType}`)
}

async function handleToggle(row: NotificationRule) {
  row.status = row.status === 'active' ? 'disabled' : 'active'
  ElMessage.success(row.status === 'active' ? '规则已启用' : '规则已停用')
  await refresh()
}
</script>
