...
import type { PrintTemplateCategory } from '@/api/system'
import type { FormColumnItem, TableColumnItem } from 'gi-component'
import { useRouter } from 'vue-router'
import CrudPage from '@/components/crud/CrudPage/index.vue'
import CrudRowActions from '@/components/crud/CrudRowActions/index.vue'
import { useTable } from '@/hooks/useTable'
import { buildPrintTemplateCategoryRows, buildPrintTemplateCategoryTree, sysPrintTemplateCategoryRecords } from '../static-data'
...
const filteredRecords = computed(() =>
  buildPrintTemplateCategoryRows().filter(
    (item) => !queryParams.keyword || item.name.includes(queryParams.keyword) || item.code.includes(queryParams.keyword)
  )
)
...
const { tableData, pagination, loading, search, refresh, onDelete } = useTable<PrintTemplateCategory>({
  ...
  deleteAPI: async (ids) => {
    ...
    while (changed) {
      changed = false
      sysPrintTemplateCategoryRecords.value.forEach((item) => {
    ...
    sysPrintTemplateCategoryRecords.value = sysPrintTemplateCategoryRecords.value.filter((item) => !deleteSet.has(item.id))
  }
})
...
async function submitDialog() {
  ...
  const parent = formModel.value.parentId ? sysPrintTemplateCategoryRecords.value.find((item) => item.id === formModel.value.parentId) : null
  const level = parent ? parent.level + 1 : 1

  if (dialogMode.value === 'add') {
    sysPrintTemplateCategoryRecords.value.unshift({
      id: `PTC-${String(sysPrintTemplateCategoryRecords.value.length + 1).padStart(3, '0')}`,
    ...
    sysPrintTemplateCategoryRecords.value = sysPrintTemplateCategoryRecords.value.map((item) =>
  ...
  await refresh()
  ElMessage.success(dialogMode.value === 'add' ? '已新增静态打印模板分类' : '已更新静态打印模板分类')
}
</script>
