...
export const AUDIT_CATEGORY_OPTIONS: StatusOption[] = [
...
export interface ProductTraceRecord {
  id: string
  trace_code: string
  trace_direction: 'forward' | 'reverse'
  status: 'linked' | 'partial' | 'isolated'
  product_code: string
  product_name: string
  serial_code: string
  wo_code: string
...
export interface ProductionLogRecord {
  id: string
  log_code: string
  status: 'captured' | 'highlighted' | 'archived'
  category: 'dispatch' | 'report' | 'release' | 'exception' | 'material'
  object_type: string
  object_code: string
  action_name: string
...
export const productTraceRecords: ProductTraceRecord[] = [
  {
    id: 'trace-chain-1',
...
export const productionLogRecords: ProductionLogRecord[] = [
  {
    id: 'log-1',
...
export function cloneFourthBatchData<T>(records: T[]): T[] {
