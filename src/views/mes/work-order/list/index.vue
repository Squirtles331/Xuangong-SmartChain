      ...
      <el-dropdown trigger="click">
        ...
        <template #dropdown>
          <el-dropdown-menu>
            ...
            <el-dropdown-item v-if="row.status === 'approved'" @click="releaseOrder(row)">下达</el-dropdown-item>
            <el-dropdown-item v-if="canReport(row)" @click="$router.push(`/mes/execution/report/${row.id}`)">报工</el-dropdown-item>
...
interface WorkOrderRow extends WorkOrder {
  current_operation_display: string
  progress: number
}
...
const searchColumns: FormColumnItem[] = [
  ...
  },
  {
    ...
    label: '生产车间',
    field: 'workshopName',
    props: { placeholder: '请输入生产车间', clearable: true } as any
  },
  {
    type: 'select-v2',
    label: '状态',
...
const { tableData, pagination, loading, search, refresh } = useTable<WorkOrderRow>({
  ...
  listAPI: async ({ page, size }) => {
    const params: WorkOrderQuery = {
      ...
      workshopName: queryParams.workshopName || undefined,
...
function mapWorkOrderRow(item: WorkOrder): WorkOrderRow {
  const currentOperationDisplay =
    item.current_operation ||
    (item.status === 'completed' ? '全部完工' : item.status === 'draft' ? '待下达' : item.status === 'approved' ? '待下达' : '待排产')

  return {
    ...item,
    current_operation_display: currentOperationDisplay,
...
function canReport(row: WorkOrderRow) {
  return row.status === 'released' || row.status === 'in_progress'
}
...
async function releaseOrder(row: WorkOrderRow) {
  ...
  ElMessage.success(response.msg || `工单 ${row.code} 已下达`)
  await refresh()
}
