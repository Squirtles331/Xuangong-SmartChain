<template>
  <gi-page-layout :bordered="true">
    ...
    <gi-table :columns="cols" :data="plans" border stripe>
      <template #level="{ row }"
        ><el-tag :type="row.level === 'I' ? 'danger' : row.level === 'II' ? 'warning' : 'info'" size="small">{{ row.level }}级响应</el-tag></template
      >
      <template #actions="{ row }"
        ><gi-button type="edit" @click="openEdit(row)" /><el-button type="primary" link size="small" @click="drill(row)">演练</el-button></template
      >
    </gi-table>
