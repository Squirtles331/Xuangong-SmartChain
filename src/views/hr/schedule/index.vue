...
const p=reactive({currentPage:1,pageSize:10,total:0});const pd=computed(()=>{p.total=data.value.length;return data.value.slice((p.currentPage-1)*p.pageSize,p.currentPage*p.pageSize)});watch(data,(v)=>{p.total=v.length},{immediate:true})
...
const formCols:FormColumnItem[]=[{type:'input',label:'班组',field:'team',required:true},{type:'select-v2',label:'班次',field:'shift',props:{options:[{label:'白班',value:'day'},{label:'夜班',value:'night'},{label:'中班',value:'middle'}]} as any},{type:'input',label:'成员',field:'members',props:{placeholder:'多人用逗号分隔'} as any},{type:'input',label:'班组长',field:'leader'}]
function openAdd(){mode.value='add';Object.assign(form,{team:'',shift:'day',members:'',leader:''});vis.value=true}
function openEdit(r:Sch){mode.value='edit';eid.value=r.id;Object.assign(form,r);vis.value=true}
async function submit(){if(!form.team){ElMessage.warning('请填写必填项');return false};if(mode.value==='add'){data.value.unshift({id:Date.now().toString(),...form} as Sch)}else{const i=data.value.findIndex(e=>e.id===eid.value);if(i>-1)Object.assign(data.value[i],form)};return true}
