  ...
  <gi-page-layout :bordered="true">
    ...
    <el-dialog v-model="pickVis" title="拣货确认" width="700px">
      <el-descriptions :column="2" border
        ><el-descriptions-item label="工单号">{{ cp?.wo_code }}</el-descriptions-item
        ><el-descriptions-item label="产品">{{ cp?.material }}</el-descriptions-item></el-descriptions
      >
      <el-table :data="pickItems" border size="small" style="margin-top: 12px">
        ...
        <el-table-column label="拣货数" width="120"
          ><template #default="{ row }"><el-input-number v-model="row.pick_qty" :min="0" :max="row.need_qty" size="small" /></template
        ></el-table-column>
      </el-table>
      <template #footer
        ><el-button @click="pickVis = false">取消</el-button><el-button type="primary" @click="confirmPick">确认拣货</el-button></template
      >
    </el-dialog>
  </gi-page-layout>
</template>
...
const picks = ref<Pick[]>([
  ...
  { id: '3', wo_code: 'WO202501140003', material: '离心泵 XJP-200', status: 'completed', created_at: '2025-01-14' }
])
...
const fd = computed(() => picks.value.filter((r) => (!s.wo_code || r.wo_code.includes(s.wo_code)) && (!s.status || r.status === s.status)))
...
const pickVis = ref(false)
const cp = ref<Pick | null>(null)
const pickItems = ref([
  { material: '泵体铸件', need_qty: 100, location: 'A-01-01', pick_qty: 100 },
  { material: '耐磨环', need_qty: 200, location: 'B-02-03', pick_qty: 200 },
  { material: '螺栓 M16×60', need_qty: 800, location: 'B-03-01', pick_qty: 800 },
  { material: '叶轮铸件', need_qty: 100, location: 'A-02-01', pick_qty: 100 },
  { material: '轴承 6308', need_qty: 200, location: 'B-04-02', pick_qty: 200 }
])
function startPick(r: Pick) {
  cp.value = r
  pickVis.value = true
}
function confirmPick() {
  if (cp.value) cp.value.status = 'picked'
  pickVis.value = false
  ElMessage.success('拣货完成')
}
function confirmOut(r: Pick) {
  r.status = 'completed'
