...
const scanCode=ref('');const records=ref<any[]>([]);const cols:TableColumnItem<any>[]=[{prop:'barcode',label:'条码',minWidth:180},{prop:'material',label:'物料',minWidth:180},{prop:'time',label:'入库时间',minWidth:170}]
function handleScanIn(){if(scanCode.value){records.value.unshift({barcode:scanCode.value,material:'45#圆钢 φ50',time:new Date().toLocaleString()});scanCode.value='';ElMessage.success('入库成功')}}
</script>
