<template>
  <gi-page-layout :bordered="true">
    ...
    <gi-table :columns="cols" :data="pd" :pagination="p" border stripe>
      <template #type="{ row }"
        ><el-tag
          :type="row.type === 'purchase' ? 'primary' : row.type === 'production' ? 'success' : row.type === 'sales' ? 'warning' : 'info'"
          size="small"
          >{{ row.type === 'purchase' ? '采购入库' : row.type === 'production' ? '生产入库' : row.type === 'sales' ? '销售退货' : '其他' }}</el-tag
        ></template
      >
      <template #status="{ row }"
      ...
      <template #actions="{ row }"
        ><el-button v-if="row.status === 'pending'" type="primary" link size="small" @click="confirmIn(row)">确认入库</el-button></template
      >
    </gi-table>
  </gi-page-layout>
</template>
...
const fd = computed(() =>
  ins.value.filter((r) => (!s.code || r.code.includes(s.code)) && (!s.type || r.type === s.type) && (!s.status || r.status === s.status))
)
