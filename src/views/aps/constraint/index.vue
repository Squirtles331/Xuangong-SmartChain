...
async function submit() {
  ...
  const list: any = { mold: molds, tool: tools, skill: skills }[curType.value]
  if (mode.value === 'add') {
    list.value.unshift({ id: Date.now().toString(), ...form })
  } else {
    const i = list.value.findIndex((e: any) => e.id === eid.value)
    if (i > -1) Object.assign(list.value[i], form)
  }
  return true
}
function del(t: string, id: string) {
  const list: any = { mold: molds, tool: tools, skill: skills }[t]
  list.value = list.value.filter((e: any) => e.id !== id)
}
</script>
