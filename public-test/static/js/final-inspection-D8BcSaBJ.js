...
import { E as W, n as ce, u as ve, v as fe } from './qms-DDSeKvwp.js'
var be = { class: 'result-panel' },
  _e = B({
    __name: 'FinalInspectionExecuteDialog',
    props: F(
      { task: {} },
      {
    ...
    emits: F(['submit'], ['update:visible', 'update:items', 'update:result']),
    setup(n, { emit: x }) {
      const p = U(n, 'visible'),
        y = U(n, 'items'),
        m = U(n, 'result'),
                ...
                l(
                  ...
                  { column: 2, border: '' },
                  {
                    default: o(() => [
                      ...
                      l(u, { label: '完工单号' }, { default: o(() => [d(g(n.task?.sourceCode), 1)]), _: 1 }),
                  ...
                  { data: y.value, border: '', style: { 'margin-top': '16px' } },
                  {
                    default: o(() => [
                      ...
                      l(
                        ...
                        {
                          default: o(({ row: r }) => [
                            l(C, { modelValue: r.value, 'onUpdate:modelValue': (R) => (r.value = R), size: 'small' }, null, 8, [
                              'modelValue',
                              'onUpdate:modelValue'
                            ])
                          ]),
  ...
  ge = L(_e, [['__scopeId', 'data-v-ead9c7f2']]),
  ye = B({
    __name: 'FinalInspectionFormDialog',
    ...
    setup(n, { emit: x }) {
      const p = U(n, 'visible'),
        ...
        V = [
          ...
          { type: 'input', label: '仓库', field: 'warehouse', required: !0 },
          { type: 'input', label: '批号', field: 'lot', required: !0 },
          { type: 'input-number', label: '数量', field: 'qty', props: { min: 1 } },
          { type: 'input', label: '完工单号', field: 'sourceCode', required: !0 },
        ...
        return (
          ...
          h(
            u,
            {
              ...
              'on-before-ok': k,
              'on-cancel': q,
              title: n.mode === 'add' ? '新增完工检验' : '编辑完工检验',
              width: '680px'
            },
            {
              default: o(() => [
    ...
    setup(n) {
      const x = [{ key: 'export', label: '导出' }],
        ...
        V = [
          ...
          { type: 'input', label: '完工单号', field: 'sourceCode', props: { clearable: !0 } },
        ...
        q = [
          ...
          { prop: 'workOrderCode', label: '工单', minWidth: 140 },
          { label: '完工单号', minWidth: 200, slotName: 'source' },
      ...
      async function K() {
        ;(C.value === 'add' ? (await ce(f.value), w.success('新增成功')) : (await W(f.value.id, f.value), w.success('编辑成功')),
          (c.value = !1),
          await u())
      }
      function Q(t) {
      ...
      async function j() {
        ...
        t && (await W(t.id, { status: 'completed', verdict: b.value }), (I.value = !1), w.success(`检验完成：${J(b.value)}`), await u())
      }
      function J(t) {
      ...
      return (t, a) => (
        ...
        h(
          ...
          },
          {
            ...
            status: o(({ row: s }) => [l(P, { value: s.status, options: p }, null, 8, ['value'])]),
            verdict: o(({ row: s }) => [l(P, { value: s.verdict || '', options: y }, null, 8, ['value'])]),
  ...
  Fe = L(Ve, [['__scopeId', 'data-v-b1249fae']])
