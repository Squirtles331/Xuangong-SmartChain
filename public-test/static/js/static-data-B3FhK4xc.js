import { Xn as a } from './element-plus-CzL7BOKu.js'
var p = a([
    {
      id: 'ROLE-001',
      code: 'platform_admin',
  ...
  c = a([
    { id: 'DICT-TYPE-001', code: 'work_order_priority', name: '工单优先级', description: '用于工单与异常任务优先级展示', status: 'active' },
    { id: 'DICT-TYPE-002', code: 'inspection_result', name: '检验结果', description: '用于 IQC/IPQC/OQC 检验结果表达', status: 'active' },
    { id: 'DICT-TYPE-003', code: 'exception_level', name: '异常等级', description: '用于执行异常与质量异常分级', status: 'disabled' }
  ]),
  u = a({
    work_order_priority: [
      {
  ...
  l = a([
    { id: 'FLOW-001', name: '普通工单审批流', businessType: 'workOrderNormal', nodes: ['车间主管', '制造经理'], status: 'active' },
    { id: 'FLOW-002', name: 'BOM 变更审批流', businessType: 'bomRouting', nodes: ['工艺工程师', 'PLM 负责人', '制造经理'], status: 'active' },
    { id: 'FLOW-003', name: '采购订单放行流', businessType: 'purchaseOrder', nodes: ['采购主管', '供应链经理'], status: 'disabled' }
  ]),
  m = a([
    {
      id: 'PARAM-001',
  ...
  v = a([
    { id: 'RULE-001', code: 'WORK_ORDER', name: '工单编码规则', prefix: 'MO', dateFormat: 'YYYYMMDD', serialLength: 4, example: 'MO202607130001' },
    {
      id: 'RULE-002',
      code: 'PURCHASE_ORDER',
      name: '采购订单编码规则',
    ...
    { id: 'RULE-003', code: 'INSPECTION_LOT', name: '检验批编码规则', prefix: 'IQC', dateFormat: 'YYMMDD', serialLength: 4, example: 'IQC2607130001' }
  ]),
  T = a([
    {
      id: 'FILE-001',
  ...
  I = a([
    ...
    { id: 'NOTICE-003', bizType: '设备异常', channel: 'internal', webhookUrl: '站内消息中心', status: 'disabled' }
  ]),
  C = a([
    {
      id: 'SSO-001',
  ...
  E = a([
    {
      ...
      templateData: { version: 'static-demo', title: '生产工单派工单', blocks: ['header', 'work-order-info', 'routing', 'footer'] },
      createdBy: 'MES实施顾问',
    ...
    },
    {
      ...
      templateData: { version: 'static-demo', title: '来料检验报告单', blocks: ['header', 'supplier', 'inspection-items', 'signature'] },
      createdBy: '质量经理',
    ...
    },
    {
      ...
      templateData: { version: 'static-demo', title: '采购送货通知单', blocks: ['header', 'supplier', 'delivery-lines', 'sign-off'] },
      createdBy: '供应链经理',
...
function M(r = o.value) {
  const s = new Map(),
    e = []
  return (
    r.forEach((t) => {
      s.set(t.id, { ...t, children: [] })
    }),
    s.forEach((t) => {
      if (!t.parentId) {
        e.push(t)
      ...
      const i = s.get(t.parentId)
      i ? (i.children = [...(i.children || []), t]) : e.push(t)
    }),
    e
  )
}
function g(r = o.value) {
  const s = new Map(r.map((e) => [e.id, e.name]))
  return r.map((e) => ({ ...e, parentName: (e.parentId && s.get(e.parentId)) || '-' }))
}
function h(r) {
  return o.value.find((s) => s.id === r)?.name || ''
}
function w(r = d.value) {
  const s = new Map(),
    e = []
  return (
    r
      .slice()
      .sort((t, i) => t.sortOrder - i.sortOrder)
      .forEach((t) => {
        s.set(t.id, { ...t, children: [] })
      }),
    s.forEach((t) => {
...
function f(r = d.value) {
  const s = new Map(r.map((e) => [e.id, e.name]))
  return r
    .slice()
    .sort((e, t) => e.sortOrder - t.sortOrder)
    .map((e) => ({ ...e, parentName: e.parentId ? s.get(e.parentId) || '-' : '根节点' }))
}
export {
  C as _,
  h as a,
